...
function collectNavigation(children: ReactNode, items: SectionNavigation[] = []) {
...
function validateNavigation(items: SectionNavigation[]) {
  ...
  for (const item of items) {
    if (!item.id.trim() || !item.title.trim()) {
      throw new Error("Section nav.id and nav.title must be non-empty strings.");
    }

    if (/\s/.test(item.id)) {
      throw new Error(`Documentation section id cannot contain whitespace: ${item.id}`);
    }
...
export function DocumentationPage({ children, className }: DocumentationPageProps) {
