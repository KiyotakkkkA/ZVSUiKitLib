...
function TimelineItem({ children, icon, className }: TimelineItemProps) {
    return (
        <div className={cn("relative grid grid-cols-[2.25rem_minmax(0,1fr)] gap-4 pb-7 last:pb-0", "after:absolute after:bottom-0 after:left-4.5 after:top-8 after:w-px after:bg-main-700 last:after:hidden", className)}>
...
function TimelineItemTitle({ children, className }: TimelineItemSectionProps) {
    return <h3 className={cn("text-sm font-semibold text-main-50", className)}>{children}</h3>;
}
...
function TimelineItemSubTitle({
    ...
    return <p className={cn("mt-1 text-xs text-main-400", className)}>{children}</p>;
}
...
function TimelineItemContent({
    ...
    return <div className={cn("mt-3 text-sm leading-6 text-main-100", className)}>{children}</div>;
}
