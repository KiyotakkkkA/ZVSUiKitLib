...
export const readThemePaletteFromCookieHeader = (
    cookieHeader?: string | null,
    cookieName = STYLE_THEME_COOKIE,
) => {
...
export const getThemeVariables = (
    ...
    const entries: Array<[string, string]> = MAIN_COLOR_STEPS.map((step) => [
        `--color-main-${step}`,
        palette.main[step],
    ]);
    for (const name of SEMANTIC_COLOR_NAMES) {
        for (const step of SEMANTIC_COLOR_STEPS)
            entries.push([`--color-${name}-${step}`, palette[name][step]]);
    }
